---
name: ghosthand
description: Use this skill when operating Ghosthand-J, a local Android control runtime exposed over a loopback HTTP API for OpenClaw or another agent. Trigger it for Ghosthand-J tasks involving runtime or capability checks, structured UI inspection, selector planning, semantic clicks, coordinate taps, text input, scrolling, wait conditions, clipboard transfer, notifications, screenshots, or debugging Ghosthand-J-specific route behavior such as partial-output warnings, snapshot-scoped node IDs, or text vs content description vs resource-id selection.
---

# Ghosthand-J

Ghosthand-J is a loopback HTTP server on the Android phone. All interaction is via HTTP `GET`, `POST`, and a small amount of `DELETE` to `http://127.0.0.1:5583`.

**Always do this first:**

| Step | Command | Purpose |
|------|---------|---------|
| 1 | `GET /ping` | Is Ghosthand-J alive? |
...
Use this skill to operate Ghosthand-J as an Android agent substrate.

Ghosthand-J is not generic Android advice. It is a local runtime with a route-based control surface. Use this skill only when the task is actually about Ghosthand-J routes, Ghosthand-J capability state, or acting through Ghosthand-J.

## What Ghosthand-J is

Ghosthand-J exposes a local HTTP API for Android observation and control. The important categories are:
...
- checking whether Ghosthand-J is running or ready
- checking whether a capability is both authorized by Android and allowed by Ghosthand-J policy
- inspecting the current Android surface before acting
- finding or clicking UI targets by `text`, `desc`, or `id`
- recovering from Ghosthand-J misses or ambiguous action results
- using Ghosthand-J to type, scroll, swipe, wait, read clipboard, or read notifications
- debugging Ghosthand-J-specific behaviors such as partial output, stale assumptions about selectors, or snapshot-scoped node IDs

Do not use it for:

- generic Android usage advice unrelated to Ghosthand-J
- root-only methods that Ghosthand-J does not expose
...
1. Is Ghosthand-J alive and usable?
...
- Ghosthand-J policy allows the capability
...
Supported strategies are `text`, `textContains`, `contentDesc`, `contentDescContains`, `resourceId`, and `focused`. `text`, `desc`, and `id` are convenience aliases in the request body; Ghosthand-J normalizes them internally.

### `/click`

Prefer `/click` over `/tap` when you have a plausible semantic target. Ghosthand-J can resolve wrapper targets, bounded selector fallbacks, and clickable ancestors, then expose how it actually landed on an actionable node.
...
For selector-based `/click`, Ghosthand-J treats `clickable=true` as the default unless you explicitly set `clickable=false`. That default is optimized for action, not inspection. Use `/find` or disable clickable resolution when you need to inspect the raw matched node.
...
When a Ghosthand-J action misses, do not branch into random retries. Make one bounded correction:
...
### Check whether Ghosthand-J is ready
...
1. confirm Ghosthand-J and capability state with `/state`
...
When summarizing a Ghosthand-J run, report only:
